...
- `mover_tarjeta` usa la **guarda atómica**: es la operación concurrente real
...
def crear_tarjeta(d) -> dict:
    ...
    # jerarquía: ancestros = camino materializado del padre + el padre
...
def mover_tarjeta(tarjeta_id, d) -> dict:
    ...
    # En el tablero, la columna Backlog significa "fuera del sprint".
    ...
    # Guarda WIP: un findOneAndUpdate sobre EL DOCUMENTO DEL PROYECTO.
    ...
    # `contaba`/`contara`: si la tarjeta entra o sale del tablero, solo
    ...
    if destino != origen:
        # El evento captura puntos y sprint DEL MOMENTO del movimiento
...
def exigir_miembro(usuario_id, proyecto_id):
    """Guarda de membresía: con un proyecto solo interactúan sus
...
def exigir_moderador(usuario_id):
    """Guarda declarativa de administración: crear/editar proyectos,
...
def es_lider(usuario_id, tarjeta) -> bool:
    """El permiso: ¿este usuario lidera la tarjeta o alguno de sus
    ancestros? UNA consulta indexada resuelve la pregunta a cualquier
    profundidad, reutilizando el materialized path: el liderazgo de un
...
def puede_archivar(usuario_id, tarjeta) -> bool:
    """Regla declarativa (sin autenticación): archivar destruye visibilidad
...
def archivar_tarjeta(tarjeta_id, usuario_id) -> dict:
    """Borrado lógico del subárbol completo: archivar un padre sin sus
    ...
    if not puede_archivar(actor, t):
        raise HTTPException(
            403, "Solo un moderador o el líder de esta rama puede archivarla")
    ts = ahora()
